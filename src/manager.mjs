...
import { atomicJson, doctor, listRoutes, resolveRoute, routeField, version } from './config.mjs';
...
/**
 * The provider/model stored when the task was created. A follow-up never
 * re-resolves the global default, so continuing a task cannot switch suppliers
 * even after the user config changed.
 */
function frozenRoute(task){
  try{return {provider:routeField(task?.provider,'task provider'),model:routeField(task?.model,'task model')};}
  catch(error){throw new Error(`Task snapshot has no usable frozen route (${error.message}); start a new task instead of continuing this one`);}
}
...
export class TaskManager extends EventEmitter {
  ...
  compactSnapshot(task,afterResultVersion=undefined){
    ...
    return {taskId:task.id,title:task.title,cwd:task.cwd,provider:task.provider,model:task.model,reasoningEffort:task.reasoningEffort,
      routeFrozen:task.routeFrozen===true,status:task.status,sessionId:task.handle?.backendSessionId,confirmedRoute:task.confirmedRoute,
  ...
  start(input){
    ...
    // A retry of one requestId is judged against the task it already created,
    // not against today's configuration: an omitted route or effort reuses the
    // frozen values, so editing the global default cannot turn the original
    // request into a different task.
    ...
    if(existing){
      ...
      const route=explicit?resolveRoute(this.config,input):frozenRoute(existing);
      const reasoningEffort=input.reasoningEffort||existing.reasoningEffort||this.config.reasoningEffort;
      // The route is part of the fingerprint. Snapshots written before routes
      // joined it hashed the task text only, so a legacy task still matches its
      // legacy digest — but only when the retry does not ask for another route.
      const digest=fingerprint({cwd,title:input.title,prompt:input.prompt,reasoningEffort,...route});
      const legacyDigest=fingerprint({cwd,title:input.title,prompt:input.prompt,reasoningEffort});
      const sameRoute=existing.provider===route.provider&&existing.model===route.model;
    ...
    // Resolve the route once, here. It is stored in the snapshot, so the route
    // is frozen with the task and every later turn reuses it verbatim.
    const route=resolveRoute(this.config,input);
    const payload={cwd,title:input.title,prompt:input.prompt,reasoningEffort:input.reasoningEffort||this.config.reasoningEffort,...route};
    if([...this.tasks.values()].filter(t=>!terminal.has(t.status)).length>=100)throw new Error('Task queue is full');
    const task={id:randomUUID(),requestId:input.requestId,fingerprint:fingerprint(payload),routeFrozen:true,...payload,
  ...
  continue(input){
    ...
    // Fail before writing a turn when this snapshot cannot name the route it was
    // created on: an unknown route must never silently become the global default.
    const route=frozenRoute(task);task.provider=route.provider;task.model=route.model;
  ...
  async run(task,turn,slot){
    ...
    try {
      // Re-assert the frozen route so a damaged snapshot fails this turn
      // instead of reaching a backend with an unset provider/model.
      const route=frozenRoute(task);task.provider=route.provider;task.model=route.model;
  ...
  async dispatch(method,args={}) {
    switch(method){
      case 'doctor':return {...doctor(this.config,{provider:args.provider,model:args.model}),controllerPid:process.pid,activeTasks:this.active.size,version,skippedTaskFiles:this.skippedTaskFiles.length};
      case 'listRoutes':return listRoutes(this.config);
      ...
      case 'list':return [...this.tasks.values()].filter(t=>!args.cwd||sameDirectory(path.resolve(args.cwd),t.cwd))
        .sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt)).slice(0,args.limit||30).map(t=>({taskId:t.id,title:t.title,cwd:t.cwd,status:t.status,sessionId:t.handle?.backendSessionId,provider:t.provider,model:t.model,updatedAt:t.updatedAt}));
